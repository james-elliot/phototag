...
fn test_latlon(path: &str) -> Option<(Option<(f64,f64)>,Option<DateTime>,i32,i32)> {
...
//			let v1:Vec<i64> =    u1.iter().map(|s| s.parse::<i64>().unwrap()).collect();
			Some(DateTime{
			    year:v0[0].to_string(),month:v0[1].to_string(),day:v0[2].to_string(),
			    hour:v1[0].to_string(),minute:v1[1].to_string(),second:v1[2].to_string()})
		    },
		    Err(_)=> {
...
fn one(p: &std::path::Path, tab: &[City], vexts: &[String], output_dir: &str, tmp_dir: &str) {
	...
	if let Some(s)=p2 {
	    ...
	    if let Some((gps, date_ret,width,height)) = test_latlon(path) {
		eprintln!("date:{:?} gps:{:?} width:{:?} height:{:?}",date_ret,gps,width,height);
		...
		let s =
		    if year != "" {
			output_dir.to_owned() + "/IMG_" + &year+&month+&day+"_"+&hour+&minute+&second + ".jpg"
		    }
		else {
		    file_name.to_string()
		};
